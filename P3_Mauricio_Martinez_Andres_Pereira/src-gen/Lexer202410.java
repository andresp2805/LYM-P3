...
public final class Lexer202410 {
  ...
  public static Object initialize() {
    ...
    $try:try {
      ...
      $line=219;
      alphaNum=$cast(ISet.class,$fix($opUnionY($opUnionY($opUnionY($opUnionY(alphaUpper,alphaLower),digits),GCollections.asSet('-')),GCollections.asSet('?'))));
      $line=221;
      Object Defn=$fix(GCollections.asSet("defvar","defun"));
      $line=222;
      Object Commds=$fix(GCollections.asSet("=","funName","move","skip","turn","face","put","pick","move-dir","run-dirs","move-face","null"));
      $line=223;
      Object Constns=$fix(GCollections.asSet("Dim","myXpos","myYpos","myChips","myBalloons","balloonsHere","ChipsHere","Spaces"));
      $line=224;
      Object Ctrols=$fix(GCollections.asSet("if","loop","repeat"));
      $line=225;
      Object Cdition=$fix(GCollections.asSet("facing?","blocked?","can-put?","can-pick?","can-move?","isZero?","not"));
      $line=227;
      keyWords=$cast(ISet.class,$fix($opUnionY($opUnionY($opUnionY($opUnionY($opUnionY(Defn,Commds),Constns),Ctrols),Cdition),GCollections.asSet("",":right",":left",":around",":front",":back","face",":north",":west",":south",":east",":balloons",":chips"))));
      $line=228;
      ...
      $line=254;
      ((gold.structures.map.GHashTableMap)translation).put("run-dirs",'R');
      $line=255;
      ((gold.structures.map.GHashTableMap)translation).put("move-face",'O');
      $line=258;
      ...
      $line=289;
      ((gold.structures.map.GHashTableMap)translation).put("loop",'o');
      $line=290;
      ((gold.structures.map.GHashTableMap)translation).put("repeat",'T');
      $line=293;
      ((gold.structures.map.GHashTableMap)translation).put("facing?",'A');
      $line=294;
      ((gold.structures.map.GHashTableMap)translation).put("blocked?",'B');
      $line=295;
      ((gold.structures.map.GHashTableMap)translation).put("can-put?",'P');
      $line=296;
      ((gold.structures.map.GHashTableMap)translation).put("can-pick?",'P');
      $line=297;
      ((gold.structures.map.GHashTableMap)translation).put("can-move?",'C');
      $line=298;
